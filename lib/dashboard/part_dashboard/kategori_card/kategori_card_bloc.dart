...
class KategoriCardBloc extends Bloc<KategoriCardEvent, KategoriCardState> {
  KategoriCardBloc() : super(KategoriCardState(
    dataKategoriCard: [],
  )) {
    on<KategoriCardEvent>((event, emit) {
      on<ViewKategoriCard>(viewKategoriCard);
    });
  }

  FutureOr<void> viewKategoriCard(ViewKategoriCard event, Emitter<KategoriCardState> emit) async{
   SharedPreferences prefs = await SharedPreferences.getInstance();
   var dataKategoriCard = <KategoriModel>[];

   emit(
     state.copyWith(
       status: KategoriCardStatus.loading
     )
   );
   ...
   try{
     ...
     for(int i=0; i<response.length; i++){
       dataKategoriCard.add(KategoriModel(luasIpal1: response[0]['luas_ipal1'],
     ...
     for(int i=0; i<dataKategoriCard.length; i++){
       print('data: ${dataKategoriCard[i].luasSegment1}');
     }
